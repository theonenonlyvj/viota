...
// (0,0) forms a 2-card line, which is always valid.
...
describe('POST /games/:id/move', () => {
  it('applies a legal move exactly once and echoes a server-derived, redacted result', async () => {
    ...
    const sync = await (await SELF.fetch(`https://example.com/games/${gameId}/sync?since=0&seat=1`)).json() as any
  ...
  it('is idempotent: replaying the same clientMoveId is a benign 200 ack, not a second row', async () => {
    ...
    const sync = await (await SELF.fetch(`https://example.com/games/${gameId}/sync?since=0&seat=0`)).json() as any
    expect(sync.moveIndex).toBe(1)
    expect(sync.moves.length).toBe(1)
  })
  ...
  it('rejects a move whose accountId does not own the seat (403)', async () => {
    const gameId = await createGame()
    const b = await firstMoveBody(gameId, 0, { x: 1, y: 0 }, uuid())
    const res = await postMove(gameId, { ...b, accountId: 'acct-imposter' })
  ...
  it('rejects an out-of-turn move (409 not_your_turn)', async () => {
    const gameId = await createGame()
    // seat 1 moves while it is seat 0's turn
  ...
  it('bounds-validates seatIndex, clientMoveId, and the move payload shape (400)', async () => {
    ...
    // out-of-range seat
    expect((await postMove(gameId, { seatIndex: 5, move: good, clientMoveId: uuid(), accountId: 'acct-0' })).status).toBe(400)
    // non-uuid clientMoveId
    expect((await postMove(gameId, { seatIndex: 0, move: good, clientMoveId: 'not-a-uuid', accountId: 'acct-0' })).status).toBe(400)
    // malformed payload (5 placements)
    const five = Array.from({ length: 5 }, (_, i) => ({ card: hand[0], position: { x: i, y: 3 } }))
    expect((await postMove(gameId, { seatIndex: 0, move: { type: 'play', placements: five }, clientMoveId: uuid(), accountId: 'acct-0' })).status).toBe(400)
    // null clientMoveId is allowed (bounds-valid); this one should succeed
    expect((await postMove(gameId, { seatIndex: 0, move: good, clientMoveId: null, accountId: 'acct-0' })).status).toBe(200)
  })
