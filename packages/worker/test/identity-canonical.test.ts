...
describe('canonical', () => {
  ...
  it('follows a merge chain to the live head', async () => {
    ...
    await acct('B', 'C')
    await acct('A', 'B')
    expect((await canonical(IDENTITY_DB(), 'A'))!.id).toBe('C')
    expect((await canonical(IDENTITY_DB(), 'C'))!.status).toBe('ghost')
  })
  it('is safe on a malformed cycle', async () => {
    await acct('X', 'Y', 'merged')
    await acct('Y', 'X', 'merged')
    const r = await canonical(IDENTITY_DB(), 'X', 8)
    expect(r).not.toBeNull() // returns last-seen without infinite loop
  })
  it('returns null for unknown id', async () => {
    expect(await canonical(IDENTITY_DB(), 'nope')).toBeNull()
  })
})
