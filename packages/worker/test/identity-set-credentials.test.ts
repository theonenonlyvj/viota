...
describe('/auth/set-credentials', () => {
  ...
  it('claims a username+password on the current ghost, in place', async () => {
    ...
    const acc = await DB()
      .prepare(`SELECT status, username, password_hash, token_epoch FROM accounts WHERE username='vee'`)
      .first<{ status: string; username: string; password_hash: string; token_epoch: number }>()
