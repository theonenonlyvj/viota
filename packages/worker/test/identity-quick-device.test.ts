...
describe('/auth/quick with device_credentials', () => {
  ...
  it('mints a ghost + a device_credentials row', async () => {
    ...
    const dev = await IDENTITY_DB().prepare(`SELECT account_id FROM device_credentials WHERE account_id=?`).bind(accountId).first()
  ...
  it('backfills a device row for a legacy account that has only credential_hash', async () => {
    ...
    const dev = await IDENTITY_DB().prepare(`SELECT credential_hash FROM device_credentials WHERE account_id='legacyacc'`).first()
