 ...
 * onward) use, resolving a token to its CANONICAL (post-merge) account.
 *
...
export type IdentityEnv = { DB: D1Database; JWT_SECRET?: string }
...
export async function requireCanonicalAccount(request: Request, env: IdentityEnv): Promise<CanonicalOk | Response> {
  ...
  const row = await env.DB.prepare(`SELECT token_epoch, status FROM accounts WHERE id=?`)
